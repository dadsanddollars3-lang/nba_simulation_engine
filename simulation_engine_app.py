        ...
        if st.button("Load All Files", use_container_width=True, type="primary"):
            ...
            for idx, uploaded_file in enumerate(uploaded_files):
                ...
                try:
                    ...
                    file_status.info(f"📖 Reading {uploaded_file.name}...")
                    df = pd.read_csv(uploaded_file)
                    
                    file_status.info(f"💾 Loading {len(df):,} rows to {table_name}...")
                    ...
                    file_status.success(f"✅ {uploaded_file.name} - Loaded {n_rows:,} rows")
                    file_progress.empty()
                    success_count += 1
                    
                except Exception as e:
                    file_status.error(f"❌ {uploaded_file.name} - Failed: {e}")
                    fail_count += 1
